...
;(function($) { 
   ...
   var dd = function(element, options)
   {
       ...
       //required-entry
        var isRequiredEntry = false;
        if($(sElement).attr("class").indexOf("required-entry") >= 0) {
            isRequiredEntry = true;
        }
        ...
        var displayorder = ($(sElement).attr("displayorder")==undefined || $(sElement).attr("displayorder")==null) ? '' : $(sElement).attr("displayorder");
	...
	var matchIndex = function (index) {
        ...
        var optionimagecreateA = function(currentOptOption, current, currentopt, tp, displayorder) {
            var aTag = "";
            ...
            if(options.useSprite!=false) {
             ...
             title = $(currentOptOption).attr("title");
             arrow = $(currentOptOption).attr("image");
            ...
            /*if(isRequiredEntry) {
                validateText = ' validate-oidropdown ';
            }
            else {
                validateText = "";
            }*/
            ...
            if(matchIndex(currentOptOption.index)==true) {
             ...
             aTag += '<a href="" class="'+styles.selected+' '+sEnabledClass+clsName+validateText+'"';
            } else {
            //aTag += '<a  href="javascript:void(0);" class="'+sEnabledClass+clsName+'"';
            aTag += '<a  href="" class="'+sEnabledClass+clsName+validateText+'"';
            };
            ...
            aTag +=  ' title="'+title+'"';
            aTag +=  ' id="'+aid+'">';
            if(displayorder == 'image' || displayorder == 'onlyimage') {
                    aTag += arrow + '<span class="'+styles.ddTitleText+'">' +sText+'</span>';
            }
            else {
                    aTag += '<span class="'+styles.ddTitleText+'">' +sText+'</span>' + arrow;
            }
            aTag += '</a>';
            return aTag;
	};
	...
	var createATags = function () {
            var childnodes = allOptions; //$(sElement) $(sElement).prop("multiple")==true)
            ...
            childnodes.each(function(current){
                 ...
                 //var displayorder = (currentOption["displayorder"]==undefined || currentOption["displayorder"]==null) ? '' : currentOption["displayorder"];
                 //OPTGROUP
                 if(currentOption.nodeName == "OPTGROUP") {
                        aTag += "<div class='opta'>";
                         ...
                         optChild.each(function(currentopt){
                            ...
                            if(displayorder) {
                                aTag += optionimagecreateA(currentOptOption, current, currentopt, "opt", displayorder);
                            }
                            else {
                      ...
                      if(displayorder) {
                         aTag += optionimagecreateA(currentOption, current, "", "", displayorder);
                      }
                      else {
	...
	var createChildDiv = function () {
            ...
            if(isRequiredEntry) {
                sDiv +=' validate-oidropdown'; //' validate-select'; //
            }
            sDiv += '"';
            if(!ddList) {
	...
	var createTitleDiv = function () {
            ...
            if(getElement(elementid).options.length>0) {
                ...
                arrow = $("#"+elementid+" option:selected").attr("image");
                arrowalt = $("#"+elementid+" option:selected").attr("title");
                if(arrow==undefined || arrow.length==0)
                {
                    arrow = $("#"+elementid+" option:selected").attr("title");
                }
            };
            arrow = (arrow==undefined || arrow.length==0 || options.showIcon==false || options.useSprite!=false) ? "" : '<img src="'+arrow+'" alt="'+arrowalt+'" align="absmiddle" /> ';
        ...
        var applyEventsOnA = function() {
            ...
            $("#"+childid+ " a.enabled").bind("click", function(event) {
                ...
                try{
                    //if(self.externBeforeClick)
                    if(typeof(window.externBeforeClick) == "function") {
                            externBeforeClick();
                    }
                } catch(e) {
                    alert("externBeforeClick: "+e.message);
                }
                 ...
                 try{
                    //if(self.externAfterClick)
                    if(typeof(window.externAfterClick) == "function") {
                            externAfterClick();
                            }
                } catch(e) {
                        //alert(e.message);
	...
	this.add = function(opt, index) {
            ...
            var title = (objOpt["title"]==undefined || objOpt["title"]==null) ? '' : objOpt["title"];
            var img = (objOpt["image"]==undefined || objOpt["image"]==null) ? '' : objOpt["image"];
            var displayorder = (objOpt["displayorder"]==undefined || objOpt["displayorder"]==null) ? '' : objOpt["displayorder"];
            ...
            if(ifA != -1) {
                //replace
                if(displayorder) {
                    aTag = optionimagecreateA(getElement(elementid).options[i], i, "", "", displayorder);
                }
                else {
                    aTag = createA(getElement(elementid).options[i], i, "", "");
                }
                $("#"+ifA.id).html(aTag);
                //a_array[key]
            } else {
