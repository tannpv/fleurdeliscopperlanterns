...
final class Mage {
     ...
     * @param string $dump 
     */
    public static function dump($dump_data, $path = null) {
        if (!$path) {
    ...
    function mydump($var, $mode = "a", $path = null) {
        /**
         * *
         * *  Description:    dump test data to file for testing
         * *  Author:         tannpv
         * *  Date:           2014/09/01   
         * $mode:   w
         *          a  
        ...
        $output = ob_get_clean();
        if (!$path) {
            $outputFile = Mage::getBaseDir() . DIRECTORY_SEPARATOR . "dump.html";
        } else {
            $outputFile = $path;
        }
        $filehandle = fopen($outputFile, $mode) or die('File creation error.');
