...
class Mage_Catalog_Block_Product_View extends Mage_Catalog_Block_Product_Abstract {
    ...
    public function getFiles() {
        ...
        foreach ($childProducts as $child) {
            $file = new stdClass();
            $item_size = $child->getAttributeText('fleur_size');
            $collection = Mage::getModel('files/files')->getFileCollection($child->getId());
            foreach ($collection->getData() as $fileCollection) {
                $path = Mage::getUrl('files/index/download' . '/id/' . $fileCollection['file_timestamp']);
                if ($fileCollection['file_type'] == 'pdf') {
                    $file->pdf = array('name' => $fileCollection['file_title'], 'path' => $path);
                } elseif ($fileCollection['file_type'] == 'cad') {
