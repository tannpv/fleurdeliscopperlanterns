...
class Mage_Sales_Model_Quote extends Mage_Core_Model_Abstract {
    ...
    Mage_Customer_Model_Customer $customer, Mage_Sales_Model_Quote_Address $billingAddress = null, Mage_Sales_Model_Quote_Address $shippingAddress = null
    ) {
        if ($customer->getId()) {
    ...
    public function getShippingAddressByCustomerAddressId($addressId) {
        foreach ($this->getAddressesCollection() as $address) {
            if (!$address->isDeleted() && $address->getAddressType() == Mage_Sales_Model_Quote_Address::TYPE_SHIPPING && $address->getCustomerAddressId() == $addressId) {
                return $address;
            }
        }
        return false;
    ...
    public function addProductAdvanced(Mage_Catalog_Model_Product $product, $request = null, $processMode = null) {
        ...
        foreach ($cartCandidates as $candidate) {
            ...
            if (!$parentItem) {
                /**
                 * *
                 * *  Description:    update fleur fields for the item
                 * *  Author:         tannpv
                 * *  Date:           2014/09/01     
                 * */
                $item = $this->updateFleurFields($item, $candidate->getCartQty());
    ...
    public function addProduct(Mage_Catalog_Model_Product $product, $request = null) {
        ...
        return $this->addProductAdvanced(
                        $product, $request, Mage_Catalog_Model_Product_Type_Abstract::PROCESS_MODE_FULL
        );
    }
    ...
    public function collectTotals() {
        ...
        Mage::dispatchEvent(
                $this->_eventPrefix . '_collect_totals_before', array(
        ...
        /**
         * *
         * *  Description:    add on price total
         * *  Author:         tannpv
         * *  Date:           2014/09/01     
         * */
        $this->setAddonBaseTotal(0);
        ...
        foreach ($this->getAllVisibleItems() as $item) {
            ...
            /**
             * *
             * *  Description:    sum up add on total for item
             * *  Author:         tannpv
             * *  Date:           date     
             * */
            $this->setAddonBaseTotal((float) $this->getAddonBaseTotal() + $item->getAddonRowTotal());
        }
        ...
        Mage::dispatchEvent(
                $this->_eventPrefix . '_collect_totals_after', array($this->_eventObject => $this)
        );
    ...
    public function updateFleurFields($item, $qty) {
        /**
         * *
         * *  Description:    update addon row total 
         * *  Author:         tannpv
         * *  Date:           2014/08/30     
         * */
        /*
          ...
          $optPrice = 0;
          foreach ($options['options'] as $selectedOpt) {
          $option = $productOpsCollection->getItemById($selectedOpt['option_id']);
          foreach ($option->getValues() as $optionValue) {
          $optPrice += $optionValue->getPrice();
          }
          } */
        $product = $item->getProduct();
        $optPrice = 0;
        ...
        $item->setAddonRowTotal($optPrice * $qty);
        $price = 0;
        if ($simpleProductType = $item->getOptionByCode('simple_product')) {
            $price = Mage::getModel('catalog/product')->load($simpleProductType->getProduct()->getId())->getPrice();
        }

        $item->setFleurBasePrice($price);
        $item->setFleurBaseRowTotal($price * $qty);
        return $item;
    }
