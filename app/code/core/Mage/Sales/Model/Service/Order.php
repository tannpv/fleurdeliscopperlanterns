...
class Mage_Sales_Model_Service_Order
{
    ...
    public function prepareInvoice($qtys = array())
    {
        ...
        foreach ($this->_order->getAllItems() as $orderItem) {
            ...
            /**
             * *
             * *  Description:    update addon_row_total, fleur_base_price, fleur_base_row_total
             * *  Author:         tannpv
             * *  Date:           date     
             **/
            $item->setAddonRowTotal($orderItem->getAddonRowTotal());
            $item->setFleurBasePrice($orderItem->getFleurBasePrice());
            $item->setFleurBaseRowTotal($orderItem->getFleurBaseRowTotal());
            $invoice->addItem($item);
        }
        /**
         * *
         * *  Description:    update invoice addon total
         * *  Author:         tannpv
         * *  Date:           2014/09/10     
         **/
   
        $invoice->setAddonBaseTotal($this->_order->getAddonBaseTotal());
