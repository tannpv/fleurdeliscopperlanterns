...
class Mage_Checkout_Model_Cart extends Varien_Object {
    ...
    protected function _getProduct($productInfo) {
        ...
        if (!$product || !$product->getId() || !is_array($product->getWebsiteIds()) || !in_array($currentWebsiteId, $product->getWebsiteIds())
        ) {
    ...
    public function updateItems($data) {
        ...
        foreach ($data as $itemId => $itemInfo) {
            ...
            /**
             * *
             * *  Description:    udpate fleur fields in the item
             * *  Author:         tannpv
             * *  Date:           2014/09/01     
             * */
            $this->getQuote()->updateFleurFields($item, $qty);
        }
    ...
    /**
     * *
     * *  Description:    get addon total from the quote
     * *  Author:         tannpv
     * *  Date:           2014/09/01q     
     * */
    public function getAddonTotal() {
//        return $this->getQuote()->getAddonBaseTotal();
        return 123;
    }
