...
class Mage_Adminhtml_Sales_OrderController extends Mage_Adminhtml_Controller_Action {
    ...
    /**
     * *
     * *  Description:    Update Shipping cost under admin
     * *  Author:         tannpv
     * *  Date:           2014/09/05     
     * */
    public function updateShippingAction() {
        ...
        if ($shipping_amount) {
            ...
            $order->setBaseShippingInclTax($shipping_amount);
            $order->setShippingInclTax($shipping_amount);
            $order->setBaseGrandTotal($baseSubTotal + $shipping_amount);
            $order->setGrandTotal($baseSubTotal + $shipping_amount);
            $order->save();
            $order->sendOrderUpdateEmail(true,'Your Order has been update with new shipping & handling charge');
            $this->_getSession()->addSuccess(Mage::helper('sales')->__('The shipping & handling fee has been updated.'));
        }
    }
