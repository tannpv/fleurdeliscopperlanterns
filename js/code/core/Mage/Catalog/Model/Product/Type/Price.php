...
class Mage_Catalog_Model_Product_Type_Price {
    ...
    public function getFinalPrice($qty=null, $product) {
     ...
     //   $finalPrice = $this->applyOptionsPriceAttr($product, $qty, $finalPrice);
...
/*
       protected function applyOptionsPriceAttr($product, $qty, $finalPrice) {
           
        if ($optionIds = $product->getCustomOption('info_buyRequest')) {
            $optionId = $optionIds->getValue();         
            $optionId = explode(';', $optionId);
            end($optionId);
            $key = key($optionId);
            $total_price_attr = explode(':', $optionId[$key-1]);
            $total_price_attr = str_replace('"', '', $total_price_attr[2]);
            if ($total_price_attr) {
                $finalPrice = $product['price'] + (int) $total_price_attr;
