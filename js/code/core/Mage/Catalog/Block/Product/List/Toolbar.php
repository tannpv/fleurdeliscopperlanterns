...
class Mage_Catalog_Block_Product_List_Toolbar extends Mage_Core_Block_Template
{
    ...
    public function getPagerHtml()
    {
     ...
     public function getHeightProducts()
    {
        $read = Mage::getSingleton('core/resource')->getConnection('core_read');  // $select = $read->select('value')->distinct()->from(array('p' => 'catalog_product_entity_varchar'))->where('attribute_id=130')->order('value DESC');
        $select = $read->select()->distinct()->from('catalog_product_entity_varchar', 'value')->where('attribute_id=130')->order('value ASC');      
        $product_height = $read->fetchAll($select);
