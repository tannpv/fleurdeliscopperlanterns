...
//        }
    ...
    /*
      public function getFledurdelistOption() {
      $optionAttrIds = $this->getBuyRequest()->getData('options');
      foreach($optionAttrIds as $k => $value){
      if($optionAttrIds[$k] == $k)
      $optionId[] = $optionAttrIds[$k];

      }
      $optionId = implode(',', $optionId);
      if($optionId){
      $read = Mage::getSingleton('core/resource')->getConnection('core_read');
      $select = $read->select('*'.'value')->from('eav_attribute_option_value')->where('option_id IN (?)', $optionId)->where('store_id = 0');
      $select1 = str_replace(array('\''), '', $select);

      $attr_name = $read->fetchAll($select1);
      }
      return $attr_name;
    ...
    /**
     * *
     * *  Description:    get custom fleur price
     * *  Author:         tannpv
     * *  Date:           2014/08/30     
     * */
    public function getFleurPrice() {
        if ($simpleProductType = $this->getOptionByCode('simple_product')) {
            $price = $simpleProductType->getProduct()->getPrice();
        } else {
    ...
    /**
     * *
     * *  Description:    get the total of the fleur price
     * *  Author:         tannpv
     * *  Date:           2014/09/01     
     * */
    public function getFleurRowTotal() {
        if ($simpleProductType = $this->getOptionByCode('simple_product')) {
            $price = $this->getFleurBaseRowTotal();
        } else {
            $price = $this->getRowTotal();
        }
        return $price;
    }
